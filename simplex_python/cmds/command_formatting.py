...
def cmd_string(cmd: BaseCommand) -> str:
    ...
    match cmd.type:
        ...
        case "apiSetActiveUser":
            view_pwd = maybe_json(cmd.get("viewPwd"))
        ...
        case "apiGetChat":
            pagination = pagination_str(cmd.get("pagination"))
        ...
        case "apiDeleteMemberChatItem":
            return f"/_delete member item #{cmd['groupId']} {cmd['groupMemberId']} {cmd['itemId']}"
        case "apiChatRead":
            item_range = ""
            if cmd.get("itemRange"):
                item_range = f" from={cmd['itemRange']['fromItem']} to={cmd['itemRange']['toItem']}"
        ...
        case "apiUpdateGroupProfile":
            return (
                f"/_group_profile #{cmd['groupId']} {json.dumps(cmd['groupProfile'])}"
            )
        ...
        case "apiVerifyContact":
            conn_code = maybe(cmd.get("connectionCode"))
        ...
        case "apiSetProfileAddress":
            return (
                f"/_profile_address {cmd['userId']} {on_off(cmd['includeInProfile'])}"
            )
        case "apiAddressAutoAccept":
            return f"/_auto_accept {cmd['userId']} {auto_accept_str(cmd['autoAccept'])}"
        case "receiveFile":
            file_path = f" {cmd['filePath']}" if cmd.get("filePath") else ""
